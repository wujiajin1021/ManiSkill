...
from mani_skill.utils.wrappers.rbs_record import RBSRecordEpisode
...
class Args:
    ...
    """Whether to record per-segmentation-id poses (world pose and camera-frame pose) into each saved trajectory under `id_poses`.
    Supported in both CPU and GPU parallel replay.
    """
    record_id_mesh_info: bool = False
    """Whether to record per-segmentation-id mesh file references and reconstructable geometry parameters.
    This stores lightweight metadata (paths/params), not mesh vertices/faces.
    """
    pointflow_npy: Optional[str] = None
    """Optional path to a scene pointflow .npy file with shape (T,H,W,3) or (T,N,3).
    When provided, replay visualization will overlay this pointflow in the simulator viewer.
    """
...
def _sleep_for_replay_fps(args: Args):
    if args.replay_fps is None:
        return
    if args.replay_fps <= 0:
        return
    time.sleep(1.0 / float(args.replay_fps))
...
def replay_parallelized_sim(
    args: Args, env: RBSRecordEpisode, pbar, episodes, trajectories
):
    ...
    for episode_batch_index, episode_batch in enumerate(batches):
        ...
        if args.vis:
            if hasattr(env, "update_pointflow_visualization"):
                env.update_pointflow_visualization()
            env.base_env.render_human()
            _sleep_for_replay_fps(args)
        ...
        if args.use_first_env_state or args.use_env_states:
            ...
            if args.record_id_poses and hasattr(env, "overwrite_last_id_pose_snapshot"):
                env.overwrite_last_id_pose_snapshot()
            if args.save_traj:
...
def replay_cpu_sim(
    args: Args, env: RBSRecordEpisode, ori_env, pbar, episodes, trajectories
):
    successful_replays = 0
...
def main(args: Args):
    ...
    if args.vis and args.sim_backend not in CPU_SIM_BACKENDS:
        logger.warning(
            "Visualization with GPU simulation backend may fail due to batched render scene-modification limits. "
            "Automatically switching replay backend to physx_cpu for visualization."
        )
        args.sim_backend = "physx_cpu"
    ...
    if args.record_id_poses and not args.save_traj:
        logger.warning(
            "record_id_poses=True but save_traj=False. id_poses are only written to trajectory files, so this setting will have no effect."
        )
    if args.record_id_mesh_info and not args.save_traj:
        logger.warning(
            "record_id_mesh_info=True but save_traj=False. mesh metadata are only written to trajectory files, so this setting will have no effect."
        )

    if args.record_id_poses and args.num_envs != 1:
        logger.warning(
            "record_id_poses is enabled with num_envs>1. This is supported, but may increase replay overhead."
        )

    if args.pointflow_npy is not None and args.num_envs != 1:
        logger.warning(
            "pointflow visualization currently targets num_envs=1 for viewer overlay. It may not align with batched replay."
        )
    if args.pointflow_npy is not None and args.sim_backend is not None and "cpu" not in args.sim_backend:
        logger.warning(
            "pointflow overlay in replay is only supported on CPU simulation backend. "
            "On GPU backend it will be automatically disabled in the wrapper."
        )
