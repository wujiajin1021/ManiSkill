...
process_trajectory() {
  ...
  python mani_skill/trajectory/rbs_replay_trajectory.py --traj-path "$traj_path" -n 10 --count 10 --use-env-states -b "$backend" --save_traj -o 'rgb+depth+segmentation' --record-id-poses --record_id_mesh_info
  python convert_camera_depths.py "$data_dir/camera_data" --workers 19
  for d in "$data_dir/camera_data"/*; do
    python flow_compress.py compress --out_dir "$d" --delete_npy
    python point_compress.py --mode compress --seg_dir "$d" --delete-existing
  done
}
